...
function renderMatchesView() {
  ...
  const matchingMatches = state.matches
    ...
    .sort((a, b) => new Date(a.date) - new Date(b.date));
  const matches = state.selectedDate === 'all' ? matchingMatches.slice(0, 100) : matchingMatches;
  const hiddenByRegiaLimit = Math.max(0, matchingMatches.length - matches.length);
  ...
  const todayMatches = state.matches.filter(match => localDateKey(match.date) === state.today);
  const todayUpcoming = todayMatches.filter(match => match.state === 'in' || (match.state === 'pre' && new Date(match.date).getTime() > Date.now()));
  const todayCompleted = todayMatches.filter(match => match.state === 'post').length;
  ...
  return `<div class="view matches-view v4-matchday">
    ${viewHeader('PRE-MATCH COMMAND', 'Tutte le partite', 'Calendario globale: la Regia mostra i prossimi eventi; scegli una data per includere anche i risultati conclusi.', `<button class="button" id="viewRefresh">${icon('refresh')} Sincronizza</button>`)}
    <section class="matchday-console">
      ...
      <article class="console-live prematch-console"><span><i></i>PROGRAMMA DI OGGI</span><strong>${todayMatches.length} gare · ${todayUpcoming.length} da giocare</strong><small>${todayCompleted} concluse · scegli “Oggi” per vedere il programma completo</small></article>
