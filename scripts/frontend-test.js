...
(async () => {
  ...
  if (!renders.roomVerify.includes('DATA RELIABILITY LEDGER') || !renders.roomVerify.includes('DATA READINESS') || !renders.roomVerify.includes('reliability-dimensions') || !renders.roomVerify.includes('Copertura disponibilità rosa') || !renders.roomVerify.includes('Impatto sulla decisione') || (renders.roomVerify.match(/class="reliability-row /g) || []).length < 6) throw new Error('Reliability Ledger V2 non renderizzato in modo completo');
  if (!renders.roomSummary.includes('gate evidenze') || !renders.roomSummary.includes(intelligencePayload.data.reliability.readiness.label)) throw new Error('Decision Passport non integra il gate delle evidenze');
