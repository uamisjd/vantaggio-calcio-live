...
(async () => {
  ...
  if (!html.includes('VANTAGGIO') || !html.includes('/app.js?v=4.1.0') || !html.includes('/styles.css?v=4.1.0')) throw new Error('Homepage o asset V4 non validi');
  const [appJs, styles] = await Promise.all([get('/app.js?v=4.1.0', 'text'), get('/styles.css?v=4.1.0', 'text')]);
  const v4Modules = ['DAILY BRIEFING', 'MATCHDAY COMMAND', 'SIGNAL STUDIO', 'VANTAGGIO NEWSROOM', 'TABLE LAB', 'MY MATCHROOM', 'SCOUT SEARCH', 'WHAT CHANGED DESK', 'KICKOFF WATCH', 'TEAM DNA', 'RELIABILITY LEDGER'];
  ...
  if (!intelligence.ok || intel.engine?.version !== '1.1' || !intel.event || !intel.context || !intel.calendar || !intel.tactical || !intel.reliability) throw new Error('Match Intelligence API non valida');
